...
class AccountMembersClient(db.Model):
    ...
    mc_client_id = db.Column(db.Integer, db.ForeignKey(
        'account_client.client_id'), primary_key=True)
    mc_member_id = db.Column(db.BigInteger, db.ForeignKey(
        'account_member.member_id'), primary_key=True)
    mc_is_approved = db.Column(
        db.SmallInteger, default=2, comment="active(3), pending(2), blocked(1)")
