    ...
    def get(self):
        ...
        if member_data is None:
            return error_http_code(
                404, {"message": "Get me failed, no member found."})
        ...
        if member_data_relation is None:
            return error_http_code(
                404,
                {"message": "Get me failed, member not register in any app."})
        ...
        if member_data.member_apps_ids != None:
            for index, app in enumerate(member_data.member_apps_ids):
                ...
                client_approved_to_app.update({
                    "client_member_approve":
                    member_data_relation[index].mc_is_approved
                })
                list_apps.append(client_approved_to_app)

        response = member_data.to_dict()
        response.update({
            "member_apps_ids": list_apps,
            "member_phone": list_phone,
